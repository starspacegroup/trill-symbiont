<script lang="ts">
	...
	const BASE_FREQUENCIES: Record<Note, number> = {
		...
		B: 493.88,
		Eb: 311.13,
		Bb: 466.16 // Same as D# and A# respectively
	};
	...
	function getChordFrequencies(): number[] {
		...
		return chordPattern
			.map((include: number, index: number) =>
...
<div class="circle-of-fifths {isActive ? 'active' : ''}">
	...
	<div class="circle-container">
		<svg width="300" height="300" viewBox="0 0 300 300" style="padding: 10px;">
			...
			<text x="150" y="157" text-anchor="middle" class="center-label" fill="white" font-size="12"
				>I</text
			>
		</svg>
	</div>
	...
	<div class="info-panel">
		<div class="current-key">
			...
			<div class="key-signature">
				Key Signature: {KEY_SIGNATURES[selectedKey as Note] > 0
					? '+' + KEY_SIGNATURES[selectedKey as Note]
					: KEY_SIGNATURES[selectedKey as Note]}
				{Math.abs(KEY_SIGNATURES[selectedKey as Note]) === 1 ? 'accidental' : 'accidentals'}
			</div>
		</div>
