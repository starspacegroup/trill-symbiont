...
<main class="min-h-screen bg-gray-900 text-white">
	<div class="container mx-auto px-4 py-8">
		<header class="text-center mb-8">
			<h1 class="text-4xl font-bold mb-2">Trill Symbiont</h1>
			<p class="text-gray-300">A shared generative ambient music experience with Circle of Fifths synchronization</p>
		...
		<div class="max-w-4xl mx-auto mb-8 p-6 bg-gray-800 rounded-xl border border-gray-700">
			<h2 class="text-2xl font-bold text-white mb-4">How to Use Trill Symbiont</h2>
			
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				<div>
					<h3 class="text-lg font-semibold text-blue-400 mb-2">Circle of Fifths</h3>
				...
				<div>
					<h3 class="text-lg font-semibold text-purple-400 mb-2">Music Grid</h3>
					<ul class="text-gray-300 space-y-1 text-sm">
			...
			<div class="mt-4 p-4 bg-gray-700 rounded-lg">
				<h4 class="text-md font-semibold text-yellow-400 mb-2">Pro Tips</h4>
				<ul class="text-gray-300 space-y-1 text-sm">
			...
			<div class="flex flex-col lg:flex-row gap-6 items-center justify-between">
				...
				<div class="flex flex-col gap-4">
					<button
						...
						class="px-6 py-3 rounded-lg font-medium transition-colors {isSynchronized ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'}"
					>
					...
					<div class="text-sm text-gray-300 text-center">
						{#if isSynchronized}
							<p>All sounds synchronized to {selectedKey} {selectedScale}</p>
							<p class="text-xs mt-1">Scale frequencies: {scaleFrequencies.length} notes</p>
						{:else}
							<p>Free mode - each square uses its own frequency</p>
						{/if}
					</div>
				</div>
			</div>
